...
app.post('/api/v1/pokemon', (req, res) => {
    pokemonSchema.create(req.body, function (err) {
        if (err) {
            console.log(err)
        } else {
...
app.put('/api/v1/pokemon/:id', (req, res) => {
    pokemonSchema.updateOne({ id: req.params.id }, { upsert: true, runValidators: true }, function (err, res) {
        if (err) {
...
app.delete('/api/v1/pokemon/:id', async (req, res) => {
    let response = null
    await pokemonSchema.deleteOne({ id: req.params.id }, async function (err, result) {
        if (err) {
            console.log(err);
